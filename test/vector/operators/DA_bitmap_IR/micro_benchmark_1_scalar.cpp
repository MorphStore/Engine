...
int main( void ) {
    ...
    // result maps: key = selectivity, value = pair of (execution time in microsec, memory in bytes)
    std::map<double, std::pair<std::chrono::duration<float, std::micro>, size_t>> position_list_results;
    std::map<double, std::pair<std::chrono::duration<float, std::micro>, size_t>> bitmap_results;
    ...
    std::map<double, std::pair<std::chrono::duration<float, std::micro>, size_t>>::iterator it_pl;
    for(it_pl = position_list_results.begin(); it_pl != position_list_results.end(); it_pl++){
        mapStream << "select. = " << (*it_pl).first
            << ": exec.(μs) = " << (*it_pl).second.first.count()
            << ", memory(B) = " << (*it_pl).second.second
            << "\n";
    }
    mapStream << "Bitmap: " << "\n";
    std::map<double, std::pair<std::chrono::duration<float, std::micro>, size_t>>::iterator it_bm;
    for(it_bm = bitmap_results.begin(); it_bm != bitmap_results.end(); it_bm++){
        mapStream << "selectivity = " << (*it_bm).first
                  << ": exec. (μs) = " << (*it_bm).second.first.count()
