...
namespace vectorlib {
   ...
   struct logic<avx2<v256<T>>, avx2<v256<T>>::vector_helper_t::size_bit::value > {
      ...
      MSV_CXX_ATTRIBUTE_FORCE_INLINE
      static
      typename avx2<v256<T>>::mask_t
      bitwise_and( typename avx2<v256<T>>::mask_t p_In1, typename avx2<v256<T>>::mask_t p_In2) {
         return ( p_In1 & p_In2 );
      }
      ...
      static
      typename avx2<v256<T>>::mask_t
      bitwise_or( typename avx2<v256<T>>::mask_t p_In1, typename avx2<v256<T>>::mask_t p_In2) {
         return ( p_In1 | p_In2 );
      }
   };
