...
class column {
   ...
   public:
      ...
      if (m_NeedsCleaning)
      {
         numa_free( m_DataUnaligned, m_MetaData.m_SizeAllocByte);
         //nvram_free( m_DataUnaligned );
      }
#else
   ...
   private:
      ...
      // Separate uncompressed chunk
      void * m_DataUncomprUnaligned;
#endif
      voidptr_t m_Data;
      // Separate uncompressed chunk
      voidptr_t m_DataUncompr;
      ...
      mutable bool m_IsNVRAM;
      mutable bool m_NeedsCleaning;
   ...
   public:
...
#ifdef MSV_NO_SELFMANAGED_MEMORY
         m_DataUnaligned{
            numa_alloc_onnode( get_size_with_alignment_padding( p_SizeAllocatedByte), numa_preferred() )
            //nvram_alloc( get_size_with_alignment_padding( p_SizeAllocatedByte ) )
         },
         m_Data{ create_aligned_ptr( m_DataUnaligned ) },
         m_DataUncomprUnaligned{ nullptr },
         m_DataUncompr{ create_aligned_ptr( m_DataUncomprUnaligned ) },
#else
         m_Data{
            ( p_PersistenceType == storage_persistence_type::globalScope )
            ? general_memory_manager::get_instance( ).allocate( p_SizeAllocatedByte )
            : numa_alloc_onnode( p_SizeAllocatedByte, numa_preferred() )
         },
#endif
      ...
      // Everuthing should be aligned
      // Non-allocating constructor. Intended for the substitution of raw buffers.
      column(voidptr_t compressed, voidptr_t uncompressed, size_t p_LogicalSize, size_t p_SizeDataByte, size_t p_SizeAllocatedByte, size_t p_SizeUncomprDataByte, bool p_IsNVRAM):
         m_MetaData{ p_LogicalSize, p_SizeDataByte + p_SizeUncomprDataByte, p_SizeDataByte, p_SizeAllocatedByte + 4096 },  //p_SizeAllocatedByte + UNCOMPR_SIZE
         m_DataUnaligned{ compressed },
         m_Data{ create_aligned_ptr( m_DataUnaligned ) },
         m_DataUncomprUnaligned{ uncompressed },
         m_DataUncompr{ create_aligned_ptr( m_DataUncomprUnaligned ) },
         m_PersistenceType{ storage_persistence_type::globalScope },
         m_IsPreparedForRndAccess(false),
         m_IsNVRAM(p_IsNVRAM), // TODO
         m_NeedsCleaning(false)
      {

      }

      inline virtual voidptr_t get_data( void ) const {
         return m_Data;
      }
      inline virtual voidptr_t get_data( void ) {
         return m_Data;
      }
      ...
      inline void set_persistence_type( storage_persistence_type p_PersistenceType ) {
         m_PersistenceType = p_PersistenceType;
      }
      inline storage_persistence_type get_persistence_type( void ) const {
      ...
      inline void set_meta_data(
         ...
         {
             // Cache processing
             nvram_flush(m_Data, p_SizeUsedByte);
         }
      }
      inline void set_meta_data( size_t p_CountValues, size_t p_SizeUsedByte )  {
        ...
        inline const voidptr_t get_data_uncompr_start() const {
            ...
            {
            return voidptr_t(create_data_uncompr_start(
                    static_cast<uint8_t *>(m_DataUncompr)
            ));
            }
        }
        ...
        inline size_t get_count_values_uncompr() const {
            ...
            {
                return (m_MetaData.m_SizeUsedByte - m_MetaData.m_SizeComprByte) / sizeof(uint64_t);
            }
        }
      ...
      static column< F > * create_global_column(size_t p_SizeAllocByte) {
         return new
            (
               ...
               //numa_alloc_onnode( sizeof( column< F > ), numa_preferred() )
#else
