...
namespace morphstore {
   ...
   template<class VectorExtension, class HashSet>
   struct semi_join_build_batch {
      ...
      MSV_CXX_ATTRIBUTE_FORCE_INLINE static void apply(
         ...
         auto state = hs.get_lookup_insert_strategy_state< VectorExtension >();
         for(size_t i = 0; i < p_Count; ++i) {
            hs.insert<VectorExtension>(
      ...
      apply(
         ...
         for( size_t i = 0; i < p_Count; ++i ) {
            ...
            compressstore<VectorExtension, iov::UNALIGNED, vector_size_bit::value>(
               outPosPtr, positionVector, lookupResultMask);
            outPosPtr += hitResultCount;
   ...
   struct semi_join {
      ...
      apply(
         ...
         semi_join_build_batch<VectorExtension, HashSet>::apply( inBuildDataPtr, buildVectorCount, hs );
//         semi_join_build_batch<scalar<base_t>, HashSet>::apply( inBuildDataPtr, buildRemainderCount, hs );
         size_t resultCount =
            semi_join_probe_batch<VectorExtension, HashSet>::apply( inProbeDataPtr, probeVectorCount, outPtr, hs );
//         resultCount +=
