...
/**
 * @file agg_sum_all.h
 * @brief Implementation of vectorized, uncompressed agg_sum_all operator.
 */
...
namespace morphstore {
	...
	struct agg_sum_processing_unit {
		...
		static void
		apply(vector_t const & p_DataVector, state_t & p_State) {
		...
		MSV_CXX_ATTRIBUTE_FORCE_INLINE
		static base_t
		finalize(typename agg_sum_processing_unit<VectorExtension>::state_t & p_State) {
		...
		apply(
		  ...
		  typename agg_sum_processing_unit<VectorExtension>::state_t & p_State
			...
			for (size_t i = 0; i < p_Count; ++ i) {
				...
				agg_sum_processing_unit<VectorExtension>::apply(dataVector, p_State);
...
#endif //MORPHSTORE_CORE_OPERATORS_UNCOMPR_AGG_SUM_ALL_H
