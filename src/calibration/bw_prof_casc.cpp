...
// ****************************************************************************
// @todo Do not repeat these in multiple source files.
...
const column<uncompr_f> * measure_morphs(
        const column<uncompr_f> * p_InCol, size_t p_CountValuesLarge
) {
    const size_t countValuesSmall = p_InCol->get_count_values();
    
    MONITORING_START_INTERVAL_FOR(
            "runtime compr [µs]",
            veName<t_vector_extension>, formatName<t_format>, t_Bw, countValuesSmall, p_CountValuesLarge
    );
...
int main(void) {
    ...
    using varex_t = variant_executor_helper<1, 1, size_t>::type
    ...
    for(unsigned bw = 1; bw <= std::numeric_limits<uint64_t>::digits; bw++) {
        ...
        varex.execute_variants(
                variants, bw, countValuesSmall, origCol, countValuesLarge
        );
