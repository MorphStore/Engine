...
/**
 * @file micro_benchmark_4_uniform_sorted_unique_scalar.cpp
 ...
 *                  - bitmap vs position-list, scalar execution
 *                  - Measure execution time
 *                  - Results are written to: micro_benchmark_4_uniform_sorted_unique_scalar.csv
 */
...
int main( void ) {
    ...
    std::unordered_map<double, std::chrono::microseconds> merge_bm_scalar;
    std::unordered_map<double, std::chrono::microseconds> merge_pl_scalar;

    std::unordered_map<double, std::chrono::microseconds> intersect_bm_scalar;
    std::unordered_map<double, std::chrono::microseconds> intersect_pl_scalar;
    ...
    for(auto selectivity : selectivities) {
        ...
        // store result:
        intersect_pl_scalar.insert({selectivity, intersect_pl_exec_time});
        ...
        // store result:
        merge_pl_scalar.insert({selectivity, union_pl_exec_time});
        ...
        // store result:
        intersect_bm_scalar.insert({selectivity, intersect_bm_exec_time});
        ...
        // store result:
        merge_bm_scalar.insert({selectivity, union_bm_exec_time});
        (void)union_bm; // satisfy compiler error
    }

    // --------------- Write results to file ---------------
    std::ofstream mapStream;
    mapStream.open("micro_benchmark_4_uniform_sorted_unique_scalar.csv");

    // pl-intersect
    mapStream << "\"PL_intersection:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : intersect_pl_scalar){
    ...
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : intersect_bm_scalar){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
    ...
    mapStream << "\"PL_merge:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : merge_pl_scalar){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
    ...
    mapStream << "\"BM_merge:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : merge_bm_scalar){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
