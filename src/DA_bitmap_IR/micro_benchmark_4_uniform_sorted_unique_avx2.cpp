...
/**
 * @file micro_benchmark_4_uniform_sorted_unique_avx2.cpp
 ...
 *                  - bitmap vs position-list, avx2 execution
 *                  - Measure execution time
 *                  - Results are written to: micro_benchmark_4_uniform_sorted_unique_avx2.csv
 */
...
int main( void ) {
    ...
    std::unordered_map<double, std::chrono::microseconds> merge_bm_avx2;
    std::unordered_map<double, std::chrono::microseconds> merge_pl_avx2;

    std::unordered_map<double, std::chrono::microseconds> intersect_bm_avx2;
    std::unordered_map<double, std::chrono::microseconds> intersect_pl_avx2;
    ...
    for(auto selectivity : selectivities) {
        ...
        // store result:
        intersect_pl_avx2.insert({selectivity, intersect_pl_exec_time});
        ...
        // store result:
        merge_pl_avx2.insert({selectivity, union_pl_exec_time});
        ...
        // store result:
        intersect_bm_avx2.insert({selectivity, intersect_bm_exec_time});
        ...
        // store result:
        merge_bm_avx2.insert({selectivity, union_bm_exec_time});
        (void)union_bm; // satisfy compiler error
    }

    // --------------- Write results to file ---------------
    std::ofstream mapStream;
    mapStream.open("micro_benchmark_4_uniform_sorted_unique_avx2.csv");

    // pl-intersect
    mapStream << "\"PL_intersection:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : intersect_pl_avx2){
    ...
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : intersect_bm_avx2){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
    ...
    mapStream << "\"PL_merge:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : merge_pl_avx2){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
    ...
    mapStream << "\"BM_merge:\"" << "\n";
    mapStream << "\"selectivity\",\"execution time (μs)\"" << "\n";
    for(auto& element : merge_bm_avx2){
        mapStream << element.first
                  << "," << element.second.count()
                  << "\n";
    }
