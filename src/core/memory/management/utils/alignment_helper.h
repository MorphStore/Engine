...
template<typename T>
MSV_CXX_ATTRIBUTE_FORCE_INLINE T* create_aligned_ptr(T* p_Ptr) {
   trace( "[] -  IN. ( ptr = ", reinterpret_cast<const void *>(p_Ptr), " )." );
   ...
   if(MSV_CXX_ATTRIBUTE_LIKELY(offset)) {
      ...
      return reinterpret_cast<T*>(ptrToSizeT + (MSV_MEMORY_MANAGER_ALIGNMENT_BYTE - offset));
   }
   ...
   return reinterpret_cast<T*>( ptrToSizeT + ( (offset!=0) * (MSV_MEMORY_MANAGER_ALIGNMENT_BYTE - offset) ) );
#else
   trace(
      "[] - OUT. ( ptr = ",
   ...
   return reinterpret_cast<T*>( ptrToSizeT + MSV_MEMORY_MANAGER_ALIGNMENT_BYTE + (-offset | MSV_MEMORY_MANAGER_ALIGNMENT_TWOS_COMPLEMENT) );
#endif
}
