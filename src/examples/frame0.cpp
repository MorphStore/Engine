...
//        CPU_SET(td->core + td->nodeSize, &cpuset);
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds((dis(gen)*100) + ((dis(gen))+5)*10));
...
void query5(ThreadData* td)
{
    ...
    //auto res = agg_sum_repl_t<ps>::apply(replicated_data);
    //cout<<"start"<<std::endl;
    append<ps>(replicated_data, std::numeric_limits<uint32_t>::max(), 1*1024*1024);  // 1 MiB append //std::numeric_limits<uint32_t>::max()
    //print_columns(print_buffer_base::decimal, res, "SUM(baseCol2)");
...
for (size_t c = 0; c < 8; c++)
{
    ...
    //replicated_data = ALUtils::allocate(dataCount * sizeof(uint64_t) * 8, setting);
    ...
    //append<scalar<v64<uint64_t>>>(replicated_data, 200, dataCount));
  ...
  for (size_t l = 0; l < 11; l++) //11
  {
    ...
    {
      replicated_data = ALUtils::allocate(dataCount * sizeof(uint64_t) * 40, setting);

      //append<avx512<v512<uint64_t>>>(replicated_data, 255, dataCount);
      append<scalar<v64<uint64_t>>>(replicated_data, std::numeric_limits<uint32_t>::max(), dataCount);
    // ------------------------------------------------------------------------
