...
void query6(ThreadData* td);
void query7(ThreadData* td);
void query8(ThreadData* td);
void query9(ThreadData* td);
...
void* query_executor(void* parameters)
{
    ...
    while (!td->halt)
    {
       // Pick up randomly or use specified query for execution
       size_t random = td->queryNum; //dis(gen);
       queries[random](td);
       counters[random]++;
       td->runsExecuted++;
    }
...
void query3(ThreadData* td)
{
    ...
    auto res = my_select_repl_wit_t<vectorlib::less, ps, uncompr_f>::apply(replicated_data_snapshot, dataCount/100, dataCount/80);
...
void query6(ThreadData* td)
{
    // Aggregation, uniform (0, dataCount)
    // Query program.
    using ps = scalar<v64<uint64_t>>;
...
void query7(ThreadData* td)
{
    // Selection 10%, uniform (0, dataCount)
    // Query program.
    using ps = scalar<v64<uint64_t>>;
    ...
    auto res = my_select_repl_wit_t<vectorlib::less, ps, uncompr_f>::apply(replicated_data_snapshot, dataCount/10, dataCount/7);
...
void query8(ThreadData* td)
{
    // Aggregation, uniform (0, 255)
    // Query program.
    using ps = scalar<v64<uint64_t>>;

    auto replicated_data_snapshot = ALUtils::take_snapshot(replicated_data3);
...
void query9(ThreadData* td)
{
    ...
    auto res = my_select_repl_wit_t<vectorlib::less, ps, uncompr_f>::apply(replicated_data_snapshot, 25, dataCount/7);
...
int main() {
    ...
    queries.push_back(&query6);
    queries.push_back(&query7);
    queries.push_back(&query8);
    queries.push_back(&query9);
...
for (size_t c = 0; c < 1; c++)
{
    ...
    replicated_data3 = ALUtils::allocate(dataCount * sizeof(uint64_t) * 2, setting);
    ...
    //data3 = generate_with_distr(dataCount, std::uniform_int_distribution<uint64_t>(0, 255), false);
    ...
    for (size_t k = 0; k < dataCount; k++)
    {
       ...
       //append<scalar<v64<uint64_t>>>(replicated_data3, ((uint64_t*)data3->get_data())[k], 1);
  ...
  for (size_t l = 4; l < 5; l++)
  {
    ...
    waitingThreads = j;
    // if PCM thread is active
    //waitingThreads = j+1;
    std::vector<ThreadData> threadParameters(j);
    // Memory bandwidth snooping PCM thread
